...
def extract_pre_post_natal_from_endt_11b(text: str) -> Dict:
    ...
    if re.search(r'pre.*?natal.*?post.*?natal', analysis_text, re.IGNORECASE) or \
       ...
       re.search(r'Pre-natal and post-natal expenses are covered', analysis_text, re.IGNORECASE):
        ...
        # Check for "Pre-natal and post-natal expenses are covered" (specific pattern from Endt. 11)
        if re.search(r'Pre-natal and post-natal expenses are covered', analysis_text, re.IGNORECASE):
...
def extract_primary_data(text: str) -> List[Dict[str, str]]:
    ...
    if not maternity_limit_match:
        # Pattern for "Rs. 25000/- Per Family for Normal delivery and 35000/- for Caesarean"
        maternity_limit_match = re.search(r"Rs\.\s*([\d,]+)/-.*?Normal\s+delivery.*?([\d,]+)/-.*?Caesarean", text, re.IGNORECASE | re.DOTALL)
    if not maternity_limit_match:
        # Pattern for "limited to Rs. 25000/- Per Family for Normal delivery and 35000/- for Caesarean"
        maternity_limit_match = re.search(r"limited to Rs\.\s*([\d,]+)/-.*?Normal\s+delivery.*?([\d,]+)/-.*?Caesarean", text, re.IGNORECASE | re.DOTALL)
    if not maternity_limit_match:
        # Pattern for "Rs.75,000 for both Normal and Caesarean per Family"
        maternity_limit_match = re.search(r"Rs\.([\d,]+).*?for both Normal and Caesarean", text, re.IGNORECASE | re.DOTALL)
    if not maternity_limit_match:
        # Pattern for "limited to Rs.75,000 for both Normal and Caesarean per Family"
        maternity_limit_match = re.search(r"limited to Rs\.([\d,]+).*?for both Normal and Caesarean", text, re.IGNORECASE | re.DOTALL)
    
    if maternity_limit_match:
        # Check if this is the "for both Normal and Caesarean" pattern (only one amount)
        if "for both Normal and Caesarean" in text:
            # Same amount for both Normal and Caesarean
